...
let rec traverse_grids_and_checkw (gl: grid list) : grid option = 
  match gl with
  | [] -> None
  | g::rest -> 
    if (is_winning_grid g) then Some g else traverse_grids_and_checkw rest
...
let play_winning_move (g: grid) : grid option =
  match g with
  | [row1; row2; row3] ->
      let g1 = create_new_grids_r1 row1 row2 row3 in
      let g2 = create_new_grids_r2 row1 row2 row3 in
      let g3 = create_new_grids_r3 row1 row2 row3 in

      (* Try the winning grids generated by new moves on row1 *)
      (match traverse_grids_and_checkw g1 with
      | Some w -> Some w
      | None ->
          (* If none found for row1, try row2 *)
          (match traverse_grids_and_checkw g2 with
          | Some w -> Some w
          | None ->
              (* If none found for row3 *)
              traverse_grids_and_checkw g3))
